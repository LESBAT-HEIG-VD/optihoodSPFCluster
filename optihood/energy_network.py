...
class EnergyNetworkClass(solph.EnergySystem):
    ...
    def createNodesData(self, data, filePath, numBuildings):
        ...
        if type(electricityCost) == np.float or type(electricityCost) == np.float64:
            # for constant cost
        ...
        if "naturalGasResource" in nodesData["commodity_sources"]["label"].values:
            ...
            if type(natGasCost) == np.float or type(natGasCost) == np.float64:
                # for constant cost
    ...
    def printInvestedCapacities(self, capacitiesInvestedTransformers, capacitiesInvestedStorages):
        for b in range(len(self.__buildings)):
            ...
            if ("pv_1__" + buildingLabel, "electricityProdBus__" + buildingLabel) in capacitiesInvestedTransformers:
                pv_counter=0
                for key, value in capacitiesInvestedTransformers.items():   # iter on both keys and values                    
                    if key[0].startswith('pv_') and buildingLabel in key[0]:
                        pv_counter+=1
                for n in range(1,1+pv_counter):
            ...
            if ("heatSource_pvt_1__" + buildingLabel, "pvtConnectBus__" + buildingLabel) in capacitiesInvestedTransformers:
                pvt_counter=0
                for key, value in capacitiesInvestedTransformers.items():   # iter on both keys and values                    
                    if key[0].startswith('heatSource_pvt_') and buildingLabel in key[0]:
                        pvt_counter+=1
